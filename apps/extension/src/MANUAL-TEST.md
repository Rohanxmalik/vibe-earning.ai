...
1. Command Palette → **vibearning: Sign in** → choose **Create account** (or **Log in**) → enter an email + password (≥8 chars to register).
2. The right-hand status item flips from **`$(sign-in) vibearning · Sign in to earn`** to **`$(rocket) vibearning ₹0.00`**.
...
4. **vibearning: Sign out** clears the token (the item returns to the sign-in call-to-action).

> Note: house ads pay nothing — to see earnings move you need a *paid*, funded campaign on the surface (see the live-earnings setup). The `~/.vibearning/token` file is an alternate credential source for the standalone status-line CLI.
...
- `codex.ts` / `geminiCli.ts` are still stubs (`isAvailable() === false`); they reuse the same status-line script via `VIBEARNING_SURFACE`. Implementing each tool's native wait-state detection is a follow-up — verify against its live agent before enabling.
