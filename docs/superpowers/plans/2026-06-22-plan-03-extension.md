...
**Tech Stack:** TypeScript, esbuild (bundle), vitest (pure core), `@types/vscode`, global `fetch` (VS Code's Node ≥18). Types reused from `@kbi/shared`.
...
**Spec:** [2026-06-22-kickbacks-india-ad-marketplace-design.md](../specs/2026-06-22-kickbacks-india-ad-marketplace-design.md) §7.
...
```json
{
  ...
  "contributes": {
    "commands": [
      { "command": "kickbacks.simulateWait", "title": "Kickbacks: Simulate Wait-State (dev)" },
      { "command": "kickbacks.endWait", "title": "Kickbacks: End Wait-State (dev)" }
    ]
  },
  "scripts": {
...
Run: `pnpm install`
Expected: `@kbi/extension` deps resolve.
...
Run: `pnpm --filter @kbi/extension test -- nonce`
...
Run: `pnpm --filter @kbi/extension test -- viewTracker`
Expected: FAIL.
...
Run: `pnpm --filter @kbi/extension test -- apiClient`
Expected: FAIL.
...
Run: `pnpm --filter @kbi/extension test -- killswitch`
Expected: FAIL.
...
Run: `pnpm --filter @kbi/extension test -- mockAdapter`
Expected: FAIL.
...
Run: `pnpm --filter @kbi/extension test -- registry`
...
Run: `pnpm --filter @kbi/extension test -- "mockAdapter|registry"`
Expected: PASS.
...
Run: `pnpm --filter @kbi/extension test -- orchestrator`
...
Run: `pnpm --filter @kbi/extension lint && pnpm --filter @kbi/extension build`
...
- [ ] `pnpm --filter @kbi/extension test` — all core suites green (nonce, viewTracker, apiClient, killswitch, mockAdapter, registry, orchestrator).
- [ ] `pnpm --filter @kbi/extension lint && build` — compiles + bundles `dist/extension.js`.
