...
- [ ] **Step 3: Run to verify fail** — `pnpm --filter @kbi/api test -- ledger.service` → FAIL
...
- [ ] **Step 5: Run to verify pass** — `pnpm --filter @kbi/api test -- ledger.service` → PASS (7)
...
- [ ] **Step 4: Run** — `pnpm --filter @kbi/api test -- metrics.service` → PASS (LedgerModule must exist first; if Task 4 not yet done, temporarily the import in module will fail at e2e only — run the unit spec which uses the mock). Defer full-suite run to Task 4.
...
- [ ] **Step 6: Run the FULL api suite** — `pnpm --filter @kbi/api test`
